...
📌 Rust: Segurança de memória sem garbage collector.
...
Cada valor tem um dono (owner): Todo valor em Rust tem uma variável que é sua "dona". Quando o dono sai de escopo, o valor é liberado.
Apenas um dono mutável por vez: Um valor pode ter um dono mutável (mut) ou vários donos imutáveis (&), mas nunca ambos ao mesmo tempo.
Valores são movidos ou emprestados: Ao passar um valor, ele é movido (transferindo posse) ou emprestado (com referências & ou &mut).
...
⚡ Armadilhas do gerenciamento de memória.
...
7. Frameworks
⚡ Hyper: a base de tudo. Tide: nosso escolhido.
...
8. Modelo de Dados
🛠️ Definindo o modelo de dados para o CRUD.
...
9. Rotas CRUD
🛠️ Implementando um CRUD com Tide e HashMap.
...
Adicione variáveis de ambiente (se necessário) no painel da Railway.
Acesse a URL gerada pelo Railway para testar a API.
...
Gerenciamento de memória = 3 leis (ownership, borrowing, lifetimes).
Erros comuns = use-after-move, borrow checker, lifetimes.
CRUD = padrão para gerenciar dados.
Runtimes = Tokio (robusto), async-std (leve).
Hyper = base low-level; Tide = high-level.
Modelo MyData = Vec<u8> em HashMap.
Swagger = documentação; Railway = deploy.
